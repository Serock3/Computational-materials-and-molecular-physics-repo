...
c = GPAW('../Task2/Na_gs_110bands.gpw')

dE = 6  # maximal Kohn-Sham transition energy to consider in eV
print("loading lr")
lr = LrTDDFT(c, filename = '../Task2/LrTDDFTresults.dat')
print("discrete_spectrum")
discrete_spectrum(lr, '../Task2/discrete_spectrum_GPAW.dat')

print("dump_data")
dump_data(lr,'../Task2/dumpFile.npz')

dump = np.load('../Task2/dumpFile.npz')
print("getting stuff")
K_pp = dump['K_pp']
...
#plt.savefig('../Task2/our_spectrum.pdf')

filename1 = '../Task1/spectrum_w.06eV.dat'
# import data
print("Plotting")
data = np.loadtxt(filename1)
plt.plot(data[:,0],data[:,1],label='Using built in function (Task 1)')
plt.plot(x_t,y_t,'--',label='Solving eig. val. prob. (Task 2)')
